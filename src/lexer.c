...
char *read_file(char path[256]) {
  ...
  if (bytes_read !=
      length) { // make sure we read exactly how much we were 'told'
    free(buffer);
    fclose(F);
    return NULL;
  }
...
char peek(Lexer lexer) { return lexer.file[lexer.pos]; }

char peek_next(Lexer lexer) { return lexer.file[lexer.pos + 1]; }

char advance(Lexer *lexer) { return lexer->file[lexer->pos++]; }
...
void lexer_init(Lexer *lexer, char *source) {
  ...
  lexer->start = 0;
  lexer->line = 1;
  lexer->keywords = initMapOf(
      {"Algorithm", ALGORITHM}, {"Begin", BEGIN}, {"Var", VAR}, {"End", END},
      {"integer", TOKEN_INTEGER}, {"real", TOKEN_REAL}, {"boolean", TOKEN_BOOL},
      {"character", TOKEN_CHAR}, {"Write", WRITE});
}
...
Token next_token(Lexer *lexer) {
  while (peek(*lexer) != '\0') {
    ...
    if (c == ';') {
      advance(lexer);
      return (Token){.type = SEMICOLON, .line = lexer->line, .lexem = ";"};
    }

    if (c == ':') {
      advance(lexer);
      return (Token){.type = COLON, .line = lexer->line, .lexem = ":"};
    }

    if (c == '(') {
      advance(lexer);
      return (Token){.type = LPAR, .line = lexer->line, .lexem = "("};
    }

    if (c == ')') {
      advance(lexer);
      return (Token){.type = RPAR, .line = lexer->line, .lexem = ")"};
    }
...
TokenArray tokenize(char *source) {
  ...
  while (true) {
    if (arr.count >= arr.capacity) {
      ...
      if (!new_tokens)
        break; // bail out, caller gets whatever we collected so far
    ...
    if (tok.type == TOKEN_ERR) {
      fprintf(stderr, "ERROR : unrecognized token %s", tok.lexem);
      exit(1);
    }
...
void free_token_array(TokenArray *arr) {
  for (int i = 0; i < arr->count; i++) {
    ...
    if (type != SEMICOLON && type != COLON && type != LPAR && type != RPAR &&
        type != TOKEN_EOF) {
...
Token peek_token(Parser *p) { return p->tokens.tokens[p->current]; }

Token advance_token(Parser *p) { return p->tokens.tokens[p->current++]; }
